...
The icon is a cybernetic ouroboros: a snake forming an open circular `O` on a deep-blue rounded tile. `brand/orion-desktop-icon.png` is the approved transparent source used by the application. Generated platform assets live in `src-tauri/icons/`, and the browser-facing copy lives in `frontend/public/orion-icon.png`.

The black area outside the rounded tile is transparent, matching the desktop-icon treatment used by the related Menma and Akasha applications. `brand/sources/orion-icon.svg` is retained only as the earlier concept mark, not the current runtime source.
