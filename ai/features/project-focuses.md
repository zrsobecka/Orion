...
Selecting the goal ring shows all contributing tasks grouped by focus in the read-only `Goal and focuses` panel. Selecting a focus ring shows only that focus's contributing tasks and synchronizes the separate editable task list below. A mission path shows recent archived focuses, the current focus, and possible next feature branches. Narrow layouts order planet, progress details, path, then working panels without horizontal overflow. Transitions respect `prefers-reduced-motion`.
